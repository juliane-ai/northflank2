...
主镜像在同一端口（默认 8080）提供实盘只读看板与 OKX 模拟策略：顶部切换“实盘 / 模拟盘”，共用原有看板登录，模拟盘地址为 `/simulation/`。设置 `SIGNAL_ENABLED=true` 并配置模拟账户即可启用，云端配置见 [部署指南](docs/signal-cloud.md)。方向策略支持成本保本、滑动止盈和重复入场；模拟执行始终使用专用模拟账户，切换页面不改变交易模式。
